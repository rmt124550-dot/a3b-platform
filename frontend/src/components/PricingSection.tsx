'use client'
import { useState } from 'react'
import Link from 'next/link'

export default function PricingSection() {
  const [billing, setBilling] = useState<'monthly' | 'annual'>('monthly')

  function fmtPrice(n: number): string {
    if (n === 0) return '0'
    return n % 1 > 0.001 ? n.toFixed(2) : n.toFixed(0)
  }



  const plans = [
    {
      id: 'trial',
      name: '🎁 Prueba gratis',
      monthly: 0, annual: 0,
      border: 'border-emerald-500/30', bg: 'bg-emerald-500/5', highlight: false,
      badge: `36 días · Sin tarjeta`,
      features: [
        'Coursera — narración completa',
        'Google Translate EN → ES',
        'Web Speech API nativa del navegador',
        'Overlay de subtítulos en pantalla',
        'Kiwi Browser + Firefox Android',
      ],
      note: `Gratis durante 36 días. Luego necesitas plan PRO para continuar.`,
      cta: `Empezar 36 días gratis`,
      href: '/register',
      ctaStyle: 'bg-emerald-500 text-white hover:bg-emerald-400',
    },
    {
      id: 'pro',
      name: 'Pro',
      monthly: 4.99, annual: 39.99, annualMonthly: 3.33, savings: '33%',
      annualBadge: '🔥 2 meses gratis',
      border: 'border-[#6366f1]/60', bg: 'bg-[#6366f1]/5', highlight: true,
      badge: 'Después del trial',
      features: [
        `Todo el período de prueba incluido (36 días)`,
        'YouTube · Udemy · edX · LinkedIn',
        'Khan Academy · DataCamp',
        'DeepL — traducción de mayor calidad',
        '10 idiomas de destino',
        'Historial + diccionario personal',
        'Sync de configuración en la nube',
      ],
      cta: 'Activar PRO',
      href: '/register?plan=pro',
      ctaStyle: 'bg-[#6366f1] text-white hover:bg-[#5558e8]',
    },
    {
      id: 'team',
      name: 'Team',
      monthly: 19.99, annual: 199.99, annualMonthly: 16.67, savings: '17%',
      annualBadge: '💎 2 meses gratis',
      border: 'border-white/10', bg: 'bg-white/2', highlight: false,
      badge: 'Para equipos',
      features: [
        'Todo lo de Pro',
        'Usuarios ilimitados',
        'Dashboard de administración',
        'API access completo',
        'Soporte prioritario',
      ],
      cta: 'Activar Team',
      href: '/register?plan=team',
      ctaStyle: 'border border-white/12 text-white/70 hover:border-white/30 hover:text-white',
    },
  ]

  return (
    <div className="max-w-4xl mx-auto text-center">
      <h2 className="text-3xl font-black mb-4">Empieza gratis 36 días</h2>
      <p className="text-white/40 mb-8">
        Sin tarjeta. Sin límites ocultos. Al terminar el trial, elige PRO para continuar.
      </p>

      {/* Toggle mensual / anual */}
      <div className="inline-flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-1 mb-10">
        <button onClick={() => setBilling('monthly')}
          className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${
            billing === 'monthly' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/70'
          }`}>
          Mensual
        </button>
        <button onClick={() => setBilling('annual')}
          className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold transition-all ${
            billing === 'annual' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/70'
          }`}>
          Anual
          <span className="text-[10px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.5 rounded-full font-bold">
            -33%
          </span>
        </button>
      </div>

      <div className="grid md:grid-cols-3 gap-6 mb-8">
        {plans.map(plan => {
          const isAnnual = billing === 'annual' && plan.annual > 0
          const price    = isAnnual ? plan.annual : plan.monthly

          return (
            <div key={plan.id}
              className={`${plan.bg} border ${plan.border} rounded-2xl p-8 relative flex flex-col ${
                plan.highlight ? 'shadow-lg shadow-[#6366f1]/10' : ''
              }`}>

              {plan.highlight && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#6366f1] text-white text-[10px] font-black px-3 py-1 rounded-full whitespace-nowrap">
                  ⭐ MÁS POPULAR
                </div>
              )}

              <div className="font-black text-xl mb-3">{plan.name}</div>

              {/* Precio */}
              {plan.monthly === 0 ? (
                <div className="mb-4">
                  <span className="text-4xl font-black text-emerald-400">$0</span>
                  <span className="text-white/35 text-sm ml-1">/36 días</span>
                  <div className="mt-2 inline-block text-xs bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 px-2.5 py-1 rounded-lg font-semibold">
                    🎁 {plan.badge}
                  </div>
                </div>
              ) : (
                <div className="mb-4">
                  {isAnnual && (
                    <div className="flex items-center justify-center gap-2 mb-1.5">
                      <span className="text-white/25 line-through text-sm">${fmtPrice(plan.monthly)}/mes</span>
                      <span className="text-[10px] bg-red-500/15 text-red-400 border border-red-500/25 px-1.5 py-0.5 rounded-full font-bold">
                        -{plan.savings}
                      </span>
                    </div>
                  )}
                  <span className="text-4xl font-black">${fmtPrice(price)}</span>
                  <span className="text-white/35 text-sm ml-1">{isAnnual ? '/año' : '/mes'}</span>
                  {isAnnual && plan.annualMonthly && (
                    <div className="flex justify-center items-center gap-1.5 mt-1.5">
                      <span className="text-emerald-400 font-bold text-sm">${fmtPrice(plan.annualMonthly)}/mes equiv.</span>
                    </div>
                  )}
                  {isAnnual && plan.annualBadge ? (
                    <div className="inline-block mt-2 text-xs bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 px-2.5 py-1 rounded-lg font-semibold">
                      {plan.annualBadge}
                    </div>
                  ) : !isAnnual && (
                    <div className="text-xs text-emerald-400/80 font-semibold mt-2">
                      🎁 36 días gratis incluidos
                    </div>
                  )}
                </div>
              )}

              <div className="border-t border-white/6 mb-5" />

              <ul className="space-y-2.5 flex-1 mb-6">
                {plan.features.map(f => (
                  <li key={f} className="flex items-start gap-2 text-sm text-white/60">
                    <span className="text-emerald-400 flex-shrink-0 mt-0.5">✓</span>
                    {f}
                  </li>
                ))}
              </ul>

              {plan.note && (
                <p className="text-white/25 text-xs mb-4 bg-white/3 rounded-lg px-3 py-2">
                  ⚠️ {plan.note}
                </p>
              )}

              <Link href={plan.id === 'trial' ? plan.href : `${plan.href}${isAnnual ? '_annual' : ''}`}
                className={`block w-full py-3 rounded-xl text-sm font-bold text-center transition-all ${plan.ctaStyle}`}>
                {plan.cta}
              </Link>
            </div>
          )
        })}
      </div>

      <Link href="/pricing" className="text-white/30 text-sm hover:text-white/60 transition-colors">
        Ver todos los detalles y preguntas frecuentes →
      </Link>
    </div>
  )
}
