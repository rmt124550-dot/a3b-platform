...
export default function BillingPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl space-y-4 sm:space-y-6">
      <h1 className="text-xl sm:text-2xl font-black">Facturación</h1>

      {/* ── Estado del trial o suscripción ──────────────────── */}
      {!isPro && (
        <div className={`border rounded-2xl p-4 sm:p-6 ${
          trialExpired ? 'bg-red-500/8 border-red-500/25'
          : trialDays !== null && trialDays <= 6 ? 'bg-amber-500/8 border-amber-500/25'
          ...
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="flex-1 min-w-0">
              <div className="font-black text-base sm:text-lg mb-1">
                ...
                : trialDays === 0 ? '⏰ Tu prueba termina hoy'
                : trialDays !== null && trialDays <= 6 ? `⚠️ Termina en ${trialDays} día${trialDays===1?'':'s'}`
              ...
              {trialExpired && (
                <div className="text-white/50 text-sm">Activa PRO para recuperar el acceso completo.</div>
              )}
            </div>
          ...
          {/* Upgrade buttons — stack en mobile */}
          {!isPro && (
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              <button onClick={() => upgrade('pro')} disabled={!!checkoutLoading}
                ...
                {checkoutLoading==='pro' ? '⏳...' : '🚀 PRO — $4.99/mes'}
              </button>
              <button onClick={() => upgrade('pro_annual')} disabled={!!checkoutLoading}
                ...
                {checkoutLoading==='pro_annual' ? '⏳...' : '🔥 Anual — $39.99/año (-33%)'}
              </button>
            </div>
          )}
        </div>
      ...
      {isPro && !loading && (
        <div className="bg-[#6366f1]/8 border border-[#6366f1]/25 rounded-2xl p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <div className="font-black text-base sm:text-lg">⭐ Plan {user?.plan?.toUpperCase()} activo</div>
              {sub?.status==='trialing' && sub.trialEnd && (
                <div className="text-emerald-400 text-sm mt-1">🎁 Trial hasta el {fmt(sub.trialEnd)}</div>
              )}
            ...
            <button onClick={openPortal} disabled={portalLoading}
              className="w-full sm:w-auto bg-white/8 border border-white/12 text-white/70 font-bold px-5 py-2.5 rounded-xl hover:bg-white/12 transition-all text-sm disabled:opacity-60">
            ...
            <table className="w-full text-sm min-w-[460px]">
              <thead className="border-b border-white/5">
                <tr className="text-white/30 text-left text-xs">
                  <th className="px-4 sm:px-6 py-3 font-normal">Fecha</th>
                  <th className="px-4 sm:px-6 py-3 font-normal">Descripción</th>
                  <th className="px-4 sm:px-6 py-3 font-normal">Monto</th>
                  <th className="px-4 sm:px-6 py-3 font-normal">Estado</th>
                  <th className="px-4 sm:px-6 py-3 font-normal">Recibo</th>
                </tr>
              </thead>
              <tbody>
                {payments.map(p => (
                  <tr key={p.id} className="border-b border-white/4 hover:bg-white/2">
                    <td className="px-4 sm:px-6 py-3 text-white/50">{fmt(p.createdAt)}</td>
                    <td className="px-4 sm:px-6 py-3 text-white/70">{p.description || 'Suscripción A3B'}</td>
                    <td className="px-4 sm:px-6 py-3 font-mono">{fmtMoney(p.amount, p.currency)}</td>
                    ...
                    <td className="px-4 sm:px-6 py-3">
                      {p.receiptUrl && (
                        <a href={p.receiptUrl} target="_blank" rel="noopener noreferrer"
                          className="text-[#6366f1] text-xs hover:underline">Ver →</a>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {loading && <div className="text-white/30 text-sm py-4">Cargando facturación...</div>}

      <p className="text-white/25 text-xs">
        ¿Problemas? <a href="mailto:hello@a3bhub.cloud" className="underline hover:text-white/50">hello@a3bhub.cloud</a>
      </p>
    </div>
  )
}
