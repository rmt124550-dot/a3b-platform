...
function PricingSection() {
  ...
  return (
    <div className="max-w-4xl mx-auto text-center">
      ...
      <p className="text-white/40 mb-8">Free incluye Coursera. PRO desbloquea todo — 7 días gratis · Sin tarjeta</p>

      {/* Toggle */}
      <div className="inline-flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-1 mb-10">
        <button onClick={() => setBilling('monthly')}
          className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${billing==='monthly'?'bg-white/10 text-white':'text-white/40 hover:text-white/70'}`}>
          Mensual
        </button>
        <button onClick={() => setBilling('annual')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${billing==='annual'?'bg-white/10 text-white':'text-white/40 hover:text-white/70'}`}>
          Anual
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        {plans.map(plan => {
          ...
          return (
            <div key={plan.id} className={`${plan.bg} border ${plan.border} rounded-2xl p-8 relative flex flex-col`}>
              {plan.highlight && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#6366f1] text-[10px] font-black px-3 py-1 rounded-full text-white whitespace-nowrap">
                  ⭐ MÁS POPULAR
                </div>
              )}
                ...
                <div className="mb-5">
                  ...
                  {isAnnual && plan.annualMonthly && (
                    <div className="mt-1.5 flex items-center gap-2">
                      <span className="text-emerald-400 font-semibold text-sm">${plan.annualMonthly}/mes</span>
                      <span className="text-xs text-white/30">equivalente</span>
                    </div>
                  )}
                  ...
                  {!isAnnual && (
                    <div className="text-xs text-emerald-400/80 font-semibold mt-2">🎁 7 días gratis · Sin tarjeta</div>
                  )}
                </div>
              )}

              <ul className="space-y-2.5 text-sm text-white/55 text-left mb-8 flex-1">
                {plan.features.map(f => (
              ...
              <Link href={plan.id==='free' ? plan.href : `${plan.href}${isAnnual?'_annual':''}`}
