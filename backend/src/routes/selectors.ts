...
// Protegido con MIGRATION_TOKEN
selectorsRouter.post('/admin-verify-email', async (req, res) => {
  const token  = req.headers['x-migration-token'] ?? req.body?.token
  ...
  try {
    ...
    // Invalidar caché
    try { await redis.del(`user_cache:${userId}`) } catch {}
    ...
    logger.info({ event: 'ADMIN_EMAIL_VERIFIED', userId: user.id, email: user.email })
    res.json({ ok: true, user, message: 'Email verified and welcome email sent' })
  } catch (err: any) {
    res.status(500).json({ error: err.message })
  }
})
